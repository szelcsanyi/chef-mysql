...
action :create do
  ...
  user 'mysql' do
    ...
    system true
    action :create
    only_if do ::File.readlines('/etc/passwd').grep(/^mysql/).size <= 0 end
  end
  ...
  directory base do
	...
	unless new_resource.base_device.nil?
		bash "create-partition-for-mysql-base" do
			user "root"
	...
	package "mysql-client" do
		action :install
		not_if do ::File::exists?("/usr/bin/mysql") end
	end
	...
	bash "install_initial_db_if_empty" do
		...
		EOH
		not_if do ::File.exists?(base + '/data/mysql/user.MYD' ) end
	end
	...
	t = template base + '/tools/mysql_healthcheck.py' do
		...
		mode '0755'
		variables(
			:logfile => base + '/log/healthcheck.log',
	...
	t = template base + '/tools/mysql_binlogcleaner.sh' do
		...
		mode '0755'
		variables(
			:binlog_dir => base + '/binlog',
			#:mysql_cli => base + '/current/bin/mysql',
			:mysql_cli => 'mysql',
	...
	if new_resource.backup_minute.nil?
		minute = sleep_time = checksum.to_s.hex % 59
	else
